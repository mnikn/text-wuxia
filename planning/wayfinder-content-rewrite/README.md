...
4. **石桥镇的入口与门槛**：镇口出口只在**村中心**、且要煮过饭药（走过 `开场.停留`）之后才出现；家里门外没有这个出口。家里 ↔ 村中心 2 刻（单程），村中心 ↔ 石桥镇 64 刻（单程），一趟来回约十七个时辰。对话写在开场收尾里，不做成单独动作。
...
6. **一屏一个叙事节拍**（2026-09-17 用户定，已写进 AGENTS.md）：单个 passage 正文不超过 3 个短段、约 180 字。按这把尺子量，当前有三处超标待裁：`地点.当铺.已当`（5 段 / 214 字）、`地点.村中心`（4 段 / 173 字）、`地点.石桥镇`（5 段 / 239 字）。

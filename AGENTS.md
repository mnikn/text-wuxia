...
- 文风基准是「金庸腔」：短句、白描、少形容词；样本见 `planning/wayfinder-content-rewrite/prototypes/worldview-seed-v3.md` 第 10 节。内容按**单场景增量**推进：每轮只起草或修改一个场景；**草稿先写在对话里给人确认，确认后才写入文件**，落库试玩后再进下一场景。禁止批量生成或提前写入后续场景。
- **一屏一个叙事节拍**：单个 passage 的正文以不超过 3 个短段、约 180 个汉字为准；任一上限放不下时拆成多个 passage，用 `<<next>>` 串联，避免一屏堆叠过多文本。
- 一些即时动作不消耗时间，对于一些耗时短到一刻的动作，直接不算时间
- **选项要提示后果**：大部分带结果的选项，标签要提示后果——如「回一部分体力」「体力回满」——让玩家在选择前能比较；不确定的事件，提示可以轻一些，如「有危险」。
- **动作单独一屏出结果**：任何动作（含纯问答的氛围动作）选中后都先落到一张结果屏（`[scene result]`）出结果与结算，选项正文只写引子；结果屏回原地——地点下的动作靠 id 点分父级自动返回，共享动作用 `<<next @back>>`。可重复动作的结算写在选项上（`[settle]` 只结算首次进入，不能用在可重复动作上）。
- **结果与代价配色**：好的结果提示用绿色，坏的结果提示与预计消耗用红色。
